...
# BM # devtools::install_github("palday/coefplot2", subdir = "pkg") # not on CRAN
...
form1 <- formula(LGM ~ Y + X, data = dat2, family = binomial)
form2 <- formula(LGM ~ elev + slope, data = dat2, family = binomial)
form3 <- formula(LGM ~ t_min_y, data = dat2, family = binomial)
form4 <- formula(LGM ~ elev + slope + p_min_spr, data = dat2, family = binomial)
form5 <- formula(LGM ~ p_min_spr + t_min_y, data = dat2, family = binomial)
form6 <- formula(LGM ~ t_avg_y + p_avg_y, data = dat2, family = binomial)
form7 <- formula(LGM ~ elev + slope + t_avg_y + p_avg_y, data = dat2, family = binomial)
form8 <- formula(LGM~ . - Name, data = dat2, family = binomial) ## all variables for step-wise procedure
...
mod.8 <-   stepAIC(glm(form8, family = binomial, data = dat2))
```
...
mods <-
  list(mod.0, mod.1, mod.2, mod.3, mod.4, mod.5, mod.6, mod.7, mod.8)
